...
const {vendorRegistration,searchVendor,updateVendortoken,vendorLogout}=require('../../service/vendor');
...
const login = async (req, res, next) => {
    ...
    if (contact_no && user_password) {
        const customer = await searchVendor( contact_no )
...
const register = async (req, res, next) => {
    if (req.body.contact_no && req.body.user_password) {
        ...
        const result =await vendorRegistration(datas);
        
        let data="";
        if(result=='true')
        {
            const customer = await searchVendor(req.body.contact_no);
...
const logout = async (req, res, next) => {
    ...
    try{
        let response=await vendorLogout(req.body.user_id);
        if(response==true)
